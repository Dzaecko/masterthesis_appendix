...
csv_file_path = 'output_cvss_data3finetunedeach.csv'
...
def callLLM(model_choice, description, metric_name, metric_shortname):
    global total_time, call_count, inputTokens, promptTokens
    client = openai.OpenAI(               
                 
            )
    metric_info =""
    if metric_shortname == "AV":
        metric_info = attack_vector_info
    elif metric_shortname == "AC":
        metric_info = attack_complexity
    elif metric_shortname == "PR":
        metric_info = privileges_required
    elif metric_shortname == "UI":
        metric_info = user_interaction
    elif metric_shortname == "S":
        metric_info = scope
    elif metric_shortname == "C":
        metric_info = confidentiality_impact
    elif metric_shortname == "I":
        metric_info = integrity_impact
    elif metric_shortname == "A":
        metric_info = availability_impact
    ...
    completion = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "user",
                        "content":  (f"Extract the specific {metric_name} ({metric_shortname}) CVSS metric value from the provided CVE vulnerability description."
f"Instead of returning the complete vector string, only return the value of the specified metric, for example formatted as '{metric_shortname}:L.' "
f"This is the CVE vulnerability description: {description}. Respond only with the value for the requested metric."
f"This is the definition of the cvss metric: {metric_info}")
          },
                ],   
                temperature=0,
                top_p=1,
    ...
    metric = completion.choices[0].message.content    
    return metric

def getvecAC(description, baseSeverity):
    global total_time, call_count, unprocessed_count, inputTokens, inputTokensRAG, promptTokens, promptTokensRAG    
    return callLLM("ft:gpt-3.5-turbo-1106:personal:ac:9MjQ5vRc", description, "Attack Complexity", "AC")
def getvecAV(description, baseSeverity):
    global total_time, call_count, unprocessed_count, inputTokens, inputTokensRAG, promptTokens, promptTokensRAG    
    return callLLM("ft:gpt-3.5-turbo-1106:personal:av:9MjRZlAp", description, "Attack Vector", "AV")
def getvecPR(description, baseSeverity):
    global total_time, call_count, unprocessed_count, inputTokens, inputTokensRAG, promptTokens, promptTokensRAG    
    return callLLM("ft:gpt-3.5-turbo-1106:personal:pr:9N50WjV7", description, "Privileges Required", "PR")
def getvecUI(description, baseSeverity):
    global total_time, call_count, unprocessed_count, inputTokens, inputTokensRAG, promptTokens, promptTokensRAG    
    return callLLM("ft:gpt-3.5-turbo-1106:personal:ui:9N5fP9uH", description, "User Interaction", "UI")
def getvecS(description, baseSeverity):
    global total_time, call_count, unprocessed_count, inputTokens, inputTokensRAG, promptTokens, promptTokensRAG    
    return callLLM("ft:gpt-3.5-turbo-1106:personal:s:9N4xtGyL", description, "Scope", "S")
def getvecC(description, baseSeverity):
    global total_time, call_count, unprocessed_count, inputTokens, inputTokensRAG, promptTokens, promptTokensRAG    
    return callLLM("ft:gpt-3.5-turbo-1106:personal:c:9MjRW9df", description, "Confidentiality", "C")
def getvecA(description, baseSeverity):
    global total_time, call_count, unprocessed_count, inputTokens, inputTokensRAG, promptTokens, promptTokensRAG    
    return callLLM("ft:gpt-3.5-turbo-1106:personal:a:9N5Td3x1", description, "Availability", "A")
def getvecI(description, baseSeverity):
    global total_time, call_count, unprocessed_count, inputTokens, inputTokensRAG, promptTokens, promptTokensRAG    
    return callLLM("ft:gpt-3.5-turbo-1106:personal:i:9N4xrPhN", description, "Integrity", "I")
...
def extract_cvss_datarandomolder(base_folder,max_files=None):
    ...
    for file_path in extract_random_files(r'C:\Users\Dzaecko\Downloads\cven\cvelistV5-main\cves', max_files=200):
                                    ...
                                    for metric in data.get('containers', {}).get('cna', {}).get('metrics', []):
                                        ...
                                        if 'baseScore' in cvss_data:  # Check if baseScore is available
                                            ...
                                            for desc in descriptions:
                                                ...
                                                AV = getvecAV(desc.get('value'), cvss_data.get('baseSeverity'))
                                                AC = getvecAC(desc.get('value'), cvss_data.get('baseSeverity'))
                                                PR = getvecPR(desc.get('value'), cvss_data.get('baseSeverity'))
                                                UI = getvecUI(desc.get('value'), cvss_data.get('baseSeverity'))
                                                S = getvecS(desc.get('value'), cvss_data.get('baseSeverity'))
                                                C = getvecC(desc.get('value'), cvss_data.get('baseSeverity'))
                                                I = getvecI(desc.get('value'), cvss_data.get('baseSeverity'))
                                                A = getvecA(desc.get('value'), cvss_data.get('baseSeverity'))
                                                
                                                vector_values = [
                                                    extract_cvss_value(AV,"AV"), extract_cvss_value(AC,"AC"), extract_cvss_value(PR, "PR"), 
                                                    extract_cvss_value(UI, "UI"), extract_cvss_value(S, "S"), extract_cvss_value(C, "C"), 
                                                    extract_cvss_value(I, "I"), extract_cvss_value(A, "A")
                                                ]

                                                # Erstellen des Vector-Strings
                                                vector_string = "/".join(f"{k}:{v}" for k, v in zip(['AV', 'AC', 'PR', 'UI', 'S', 'C', 'I', 'A'], vector_values))
...
def extract_cvss_value(cvss_string, metric):
    """
    Extrahiert einen spezifischen CVSS-Wert aus einem gegebenen CVSS-String.
    :param cvss_string: Der komplette CVSS-String, z.B. 'CVSS:3.1/AV:N/AC:L/PR:H/UI:N/S:C/C:H/I:H/A:H'
    :param metric: Der Metrik-Schlüssel, dessen Wert extrahiert werden soll, z.B. 'C'
    :return: Den Wert der Metrik oder None, falls die Metrik nicht gefunden wurde.
    """
    # Der reguläre Ausdruck sucht nach dem Metrik-Schlüssel gefolgt von einem Doppelpunkt und einem Großbuchstaben.
    # Dies deckt Fälle wie 'C:H' ab.
    pattern = rf"(?<!\w){metric}:([A-Z]+)"
...
#extract_cvss_datarandomolder(r'C:\Users\Dzaecko\Downloads\cven\cvelistV5-main\cves', max_files=200)
